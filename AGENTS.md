...
- Graphics backends: `libhybris`, `libhybris-zink`, `gfxstream`, `cpu`, and `libhybris-gl4es` ship by default. `libhybris` works on all tested physical devices and is the production/default path. `gfxstream` is experimental/partial; it is the x86_64 emulator default only because libhybris is unsupported there. `libhybris-gl4es` is an edge-case, never-default backend for legacy fixed-function desktop-GL apps (`glxgears`) that `libhybris`'s GLES-only path can't run at all. See [notes/gpu-strategy.md](notes/gpu-strategy.md), [notes/libhybris-zink.md](notes/libhybris-zink.md), [notes/libhybris-gl4es.md](notes/libhybris-gl4es.md), and [notes/gfxstream-bridge.md](notes/gfxstream-bridge.md).
